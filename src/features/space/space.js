...
import { knowledgeNavigatorMarkup } from '../navigator/navigator.js';
...
export function renderSpace({ id, lens='story', graph, selectors, repository }) {
  ...
  return `<section class="space-overlay" data-space-id="${space.id}" style="view-transition-name: signal-${space.id}">
    ...
    <div class="space-body"><div class="lens-content">${content}</div>${knowledgeNavigatorMarkup({spaceId:space.id,graph})}</div>
  </section>`;
}
